...
@section('content_header_title')
    <h1 class="m-0"><b>Grupos de Usuários</b></h1>
@endsection
                            ...
                            <tr>
                                <td>
                                    @can('admin.group.show')
                                        <a href="{{ route('admin.groups.show', $group->id) }}" class="@if(auth()->user()->dark_mode) text-white @else text-dark @endif">{{ $group->title }}</a>
                                    @else
                                        {{ $group->title }}
                                    @endcan
                                </td>
